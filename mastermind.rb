...
def increment_poss_num(count, last_key, poss_num, combinations)
  ...
  if count == 1 || count == 2
    last_key.uniq.combination(count + 1).to_a.each do |e| # is combination not that effective?? product more effective?
...
def cpu_guess(prev_guesses, combinations, poss_loc, poss_num)
  ...
  #case statement counts match number, deletes combinations, and add weights to poss_num
  ...
  likely_numbers = find_likely(poss_num, last_value.count) # maybe hamstringing the program by forcing it to only return a number equal to number of matches
  likely_locations = find_likely(poss_loc, last_value.count("b")) # ie 1 match = only 1 likely number/location
  #dont guess these numbers, or these numbers at specific locations
  ...
  return guess
  #need a deduction method to compare against previous guesses? or is overkill?
end
...
def play_round(codemaker, codebreaker, p1, p2)
  board = GameBoard.new
  #pick hidden_code (random for cpu or 'gets' for player)
  hidden_code = make_hidden_code(codemaker)
  #Set all possible code combinations for CPU guess. 
  #These will be systematically removed by the CPU guessing method
  a = [1, 2, 3, 4, 5, 6, 7]
  combinations = Hash[a.product(a.product(a.product(a))).map(&:flatten).map {|b| [b, 0]}]
  #lists of numbers previous guesses to pass to CPU guess
  poss_loc = {} # ex {[6, 3] => 2}, number 6 on position (array)index 3, with a weight of 2 
  poss_num = Hash[a.map {|q| [q, 0]}]  # ex {1=>0, 2=>0, etc}
  prev_guesses = {} # ex {[1, 1, 2, 2]=>["w"]}
  ...
  while (code_broken == false && guess_number <= 12) do
    #codebreaker chooses guess
    if codebreaker.human == true
      breaker_guess = get_guess(codebreaker)
    elsif prev_guesses == {}
      breaker_guess = first_cpu_guess()
    else  
      breaker_guess = cpu_guess(prev_guesses, combinations, poss_loc, poss_num)
    end
    ...
    #update game board rows to show previous guesses and matches
...
def master_sequence
  #pick human or cpu players and codemaker & codebreaker
